...
!$$$  
      ...
      use vrbls2d, only: tmax, qrmax, htop, hbot, cuppt, fis, cfrach, cfracl,      &
              ...
              hbotd,hbots,htopd,htops,tot_edust,vis_dust
...
! For aerosol aware microphyscis
      ...
      do l = 1, lm
       do j = jsta_2l, jend_2u
        ...
        if(i.eq.im/2.and.j.eq.(jsta+jend)/2)print*,'sample QQNW= ',    &    
          i,j,l,QQNW ( i, j, l )
        end do
       end do
      end do
...
! KRF - check surface pressure for monotonic correctness
         ...
         if((PINT(I,J,lm) - PINT(I,J,LM+1)).ge.0. ) then
!           write(*,*) 'non-monotonic PINT, i,j,lm ', i,j,lm
...
! CRA DUST FROM WRF-CHEM
     VarName='TOT_DUST'
      ...
      do l=1,lm
       do j = jsta_2l, jend_2u
        do i = 1, im
            TOT_DUST ( i, j, l) = dum3d ( i, j, l )
        end do
       end do
      end do
      print*,'Read TOT_DUST, value at domain center :',TOT_DUST(im/2,jm/2,lm)

! --- Near-surface dust diagnostic for PM10-like post-processing ---
! WRF TOT_DUST is already a concentration (ug m^-3), so do NOT sum over
! the full column.  Use the lowest model level only for the 2-D hazard field.
! --- Read total dust concentration from WRF-Chem ---
! --- Dust concentration proxy and calibrated visibility diagnostic ---
      lbot = max(1, lm-9)
      ...
      do j = jsta_2l, jend_2u
        do i = 1, im
          ...
          do l = lbot, lm
            dustlev = max(dble(TOT_DUST(i,j,l)), 0.0d0)
            dust10max = max(dust10max, dustlev)
          end do

          DUSMASS(i,j) = dust10max
      ...
      call getVariable(fileName,DateStr,DataHandle,VarName,DUMMY,        &
       ...
       do j = jsta_2l, jend_2u
        do i = 1, im
! Dust emission/deposition outputs are disabled in wrf_cntrl.parm for this
! near-surface PM10 workflow, so keep DUEM inactive here.
            DUEM ( i, j, 1 ) = 0.0
        end do
       end do
      VarName='DUSTLOAD_2'
      call getVariable(fileName,DateStr,DataHandle,VarName,DUMMY,        &
       ...
       do j = jsta_2l, jend_2u
        do i = 1, im
!           DUEM ( i, j, 2 ) = dummy ( i, j )
!Radi, I changed these DUSTLOAD_.. variables to use TOT_DUST
            DUEM ( i, j, 2 ) = 0.
        end do
       end do
      VarName='DUSTLOAD_3'
...
! Use TOT_DUST as the only active dust mass field for this workflow.
            DUST ( i, j, l, 1) = TOT_DUST ( i, j, l )
        end do
       end do
      end do
     VarName='DUST_2'
...
! Disable unused dust-bin outputs for this near-surface PM10 workflow.
            DUST ( i, j, l, 2) = 0.
        end do
       end do
      end do
